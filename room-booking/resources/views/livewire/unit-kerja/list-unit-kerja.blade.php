<div class="container mx-auto">
    ...
    <div class="flex justify-between mb-4">
        <flux:button :href="route('unit-kerja.create')" variant="primary">
            New Unit Kerja
        </flux:button>
    </div>
    ...
    <table class="min-w-full border-collapse border border-gray-400 mt-4">
        ...
        <tbody>
            @foreach ($unitKerjas as $unit)
                <tr>
                    <td class="py-2 px-4 border border-gray-300">{{ $unit->id }}</td>
                    <td class="py-2 px-4 border border-gray-300">{{ $unit->kode }}</td>
                    <td class="py-2 px-4 border border-gray-300">{{ $unit->nama }}</td>
                    <td class="py-2 px-4 border border-gray-300">
                        <flux:button :href="route('unit-kerja.edit', $unit)" variant="primary">
                            Edit
                        </flux:button>
                        <flux:button variant="danger" wire:click="delete({{ $unit->id }})" wire:confirm="Are you sure?">
                            Delete
                        </flux:button>
                    </td>
                </tr>
